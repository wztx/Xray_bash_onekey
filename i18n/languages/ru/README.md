...
* **Убедитесь, что curl установлен: CentOS пользовательское выполнение.`yum install -y curl`；Debian/Ubuntu 用户执行 `apt install -y curl`
...
| Переключатель режима | Переключитесь в другой режим протокола (например, Reality → TLS). По умолчанию повторно используется только основной пользователь UUID/email. | 其他用户不自动迁移，切换前会明确提示 |
