...
재구성 프로세스의 단계 중 하나라도 실패하면(구성 작성, 서비스 시작, 상태 확인 등) 자동으로 원래 백업 구성으로 롤백됩니다. 백업 디렉터리는 고유한 타임스탬프를 사용하여 서로 충돌하지 않고 여러 번의 연속 재구성을 지원합니다.
...
* 가용성을 확인하지 않고 프로덕션 환경에서 이 스크립트를 사용하지 마십시오.
