...
* Поддерживает двойной стек IPv4 / IPv6: может автоматически определять возможности экспорта в общедоступную сеть во время установки, записывать независимую проверку на основе имени домена A/AAAA и генерировать соответствующие ссылки для общего доступа и конфигурации Clash.
...
* **确保已安装 curl**：CentOS 用户执行 `yum install -y curl`;Debian/Ubuntu Пользовательское выполнение`apt install -y curl`
...
* Решение: автоматически определить тип неисправности и дать рекомендации по ее обработке. В инструкции четко указано, что автоматическая обработка разрешена или даны только предложения.
